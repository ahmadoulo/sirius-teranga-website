...
const HeroSection = () => {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center text-center overflow-hidden bg-gradient-to-br from-[#071528] via-[#0B1F3A] to-[#102848]">
      ...
      {/* Content */}
      <div className="relative z-10 max-w-[860px] mx-auto px-8 py-32">
        <div ref={badgeRef} className="inline-flex items-center gap-2 font-heading text-xs font-bold tracking-[0.12em] uppercase text-accent bg-accent/10 border border-accent/30 px-5 py-2 rounded-full mb-8 backdrop-blur-sm">
          <span className="w-[7px] h-[7px] bg-accent rounded-full animate-pulse-dot" />
          <span>{t("hero.badge")}</span>
        </div>

        <h1 ref={titleRef} className="font-heading font-black text-[clamp(2.2rem,5.5vw,3.8rem)] leading-[1.15] text-white mb-8 tracking-tight">
          {t("hero.title")}<br />
          <span className="text-accent">{t("hero.highlight")}</span>
        </h1>

        <p ref={subRef} className="text-[clamp(1rem,2vw,1.2rem)] text-white/75 max-w-[600px] mx-auto mb-10 leading-relaxed">
          {t("hero.subtitle")}
        </p>
        ...
        <div ref={ctaRef} className="flex flex-col sm:flex-row gap-4 justify-center mb-16">
          <Link
            to="/contact"
            className="inline-flex items-center justify-center gap-2 font-heading text-sm font-bold tracking-[0.05em] uppercase bg-accent text-accent-foreground px-8 py-3.5 rounded-full border-2 border-transparent shadow-gold hover:bg-gold-light hover:-translate-y-0.5 hover:shadow-[0_12px_40px_rgba(212,175,55,0.45)] transition-all duration-300 relative overflow-hidden"
          >
            {t("hero.cta1")}
          </Link>
          <Link
            to="/services"
            className="inline-flex items-center justify-center gap-2 font-heading text-sm font-bold tracking-[0.05em] uppercase bg-transparent text-white px-8 py-3.5 rounded-full border-2 border-white/50 hover:bg-white/10 hover:border-white hover:-translate-y-0.5 transition-all duration-300"
          >
            {t("hero.cta2")}
          </Link>
        </div>
        ...
        {/* Stats bar */}
        <div ref={statsRef} className="flex flex-col sm:flex-row items-center justify-center gap-0 px-8 py-6 bg-white/5 border border-white/10 rounded-xl backdrop-blur-[12px]">
          ...
          <div className="sm:hidden w-20 h-px bg-white/20 my-4" />
...
const StatItem = ({ numKey, labelKey }: { numKey: string; labelKey: string }) => {
  ...
  return (
    <div ref={ref} className="flex flex-col items-center px-8 sm:px-12">
      <div className="flex items-baseline">
        <span className="font-heading text-[2.5rem] font-black text-white leading-none">{count}</span>
        <span className="font-heading text-[1.8rem] font-black text-accent">+</span>
      </div>
      <span className="text-[0.78rem] text-white/60 uppercase tracking-[0.08em] mt-1.5 font-heading">{t(labelKey)}</span>
    </div>
  );
};
...
const Particles = () => {
  useEffect(() => {
    // Create particles via DOM for performance
    ...
    for (let i = 0; i < 18; i++) {
      ...
      p.style.cssText = `
        position:absolute;border-radius:50%;background:#D4AF37;opacity:0;
        width:${size}px;height:${size}px;left:${x}%;bottom:-${size}px;
